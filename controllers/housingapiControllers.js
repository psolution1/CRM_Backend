...
const ServiceModel = require("../models/productserviceModel");
  ...
  try {
    ...
    for (const leadData of response.data.data) {
      ...
      let service = await ServiceModel.findOne({
        product_service_name: leadData?.project_name,
      });
      ...
      if (!existingLead) {
        await Lead.create({
          ...
          lead_date: leadData.lead_date,
          description: `Apartment: ${leadData.apartment_names}, Project name: ${leadData?.project_name}`,
        });
