  ...
  try {
    ...
    await Promise.all(
      agents.map(async (agent) => {
        ...
        const fromDateStr = moment(yesterday).format("YYYY-MM-DD");
        const toDateStr = moment(today).format("YYYY-MM-DD");
        ...
        try {
          ...
          // You might want to handle errors for individual agents here if needed
        }
      })
    );
...
////////get all call log of previous day

exports.GetAllUserCallLogByIdPreviousday = catchAsyncErrors(
  async (req, res, next) => {
    try {
      const agents = await Agent.find({ role: "user" });
