...
exports.LeadSourceReport = catchAsyncErrors(async (req, res, next) => {
  ...
  const leadSource = await Lead.find(query)
    .select("full_name lead_cost")
    .maxTimeMS(30000);
...
exports.LeadProductServiceOverviewApi = catchAsyncErrors(
  async (req, res, next) => {
    try {
      const product = await Product.find();

      const product1_id = product.map((product1) => product1._id);
      const product_name = product.map(
        (product2) => product2.product_service_name
      );
...
exports.GetProductReportDateWise = catchAsyncErrors(async (req, res, next) => {
  ...
  const leadSource = await Lead.find(query)
    .select("full_name followup_won_amount")
    .maxTimeMS(30000);
  if (!leadSource || leadSource.length === 0) {
    return next(new ErrorHander("No Data Found Now", 404));
  }
...
exports.EmployeesReportDetailByFilter1 = catchAsyncErrors(
  async (req, res, next) => {
    const {
      agent,
      service,
    ...
    const wonLeadCount = await Lead.aggregate([
      {
        $match: {
          status: new ObjectId("65a904e04473619190494482"),
          ...matchConditions,
        },
      },
      { $count: "count" },
    ...
    const ratio =
      ((wonLeadCount.length > 0 ? wonLeadCount[0].count : 0) / lead.length) *
        100 || 0; // Handle division by zero
...
/////////  Employees report By Filter
exports.EmployeesReportDetailByFilter = catchAsyncErrors(
  async (req, res, next) => {
    const {
      agent,
      service,
    ...
    if (role === "TeamLeader") {
      if (agent) {
        ...
        const ratio =
          ((wonLeadCount.length > 0 ? wonLeadCount[0].count : 0) /
            lead.length) *
            100 || 0; // Handle division by zero
