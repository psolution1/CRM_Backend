...
exports.bwnotification = catchAsyncErrors(async (req, res, next) => {
  ...
  const tokentable = await Notification.find({
    user_id: "65a8eee17dd25ded3ca0fb99",
  });
...
exports.IncomeGraphOverviewForUser = catchAsyncErrors(
  async (req, res, next) => {
...
exports.IncomeGraphOverviewForTeamLeader = catchAsyncErrors(
  async (req, res, next) => {
    const { user_id } = req.body;
...
////for Teamleader
exports.GetCalandarDataByTeamLeader = catchAsyncErrors(
  async (req, res, next) => {
    const { user_id } = req.body;
...
exports.YearlySaleApi = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    const andialTime = new Date(
      currentUTCTime + (andialTimezoneOffset + andialTimezoneOffset1)
    );
    ...
    TotalAmountWon = wonlead.reduce(
      (total, lead) => total + parseInt(lead.followup_won_amount),
      0
    );
    TotalAmountLost = lostlead.reduce(
      (total, lead) => total + parseInt(lead.lead_cost),
      0
    );
    TotalAmountwonmanthely = wonleadforthirtyday.reduce(
      (total, lead) => total + parseInt(lead.followup_won_amount),
      0
    );
...
exports.YearlySaleApiForTeamLeader = catchAsyncErrors(
  async (req, res, next) => {
    const { user_id } = req.body;
    const [agentsByAssigntl, agentsById] = await Promise.all([
      agent.find({ assigntl: user_id }).select("_id"),
    ...
    try {
      ...
      const wonlead = await Lead.find({
        status: wonStatus_id,
        assign_to_agent: { $in: agentIds },
      });
      const lostlead = await Lead.find({
        status: lostStatus_id,
        assign_to_agent: { $in: agentIds },
      });
...
exports.LeadSourceOverviewApi = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    const Lead_source_name = lead_source.map(
      (lead_source1) => lead_source1.lead_source_name
    );

    const Lead_source_countPromises = Lead_source_id.map(
      async (Lead_source_id1) => {
...
exports.LeadSourceOverviewApiForTeamLeader = catchAsyncErrors(
  async (req, res, next) => {
    const { user_id } = req.body;
    const [agentsByAssigntl, agentsById] = await Promise.all([
      agent.find({ assigntl: user_id }).select("_id"),
    ...
    try {
      ...
      const Lead_source_id = lead_source.map(
        (lead_source1) => lead_source1._id
      );
      const Lead_source_name = lead_source.map(
        (lead_source1) => lead_source1.lead_source_name
      );
...
exports.LeadSourceOverviewApiForUser = catchAsyncErrors(
  async (req, res, next) => {
    const user_id = new ObjectId(req.body.user_id);
    try {
      const lead_source = await leadsourceModel.find();
...
exports.UnAssignedDashboardLeadCount = catchAsyncErrors(
  async (req, res, next) => {
    let array = [];
    const lead = await Lead.find({ assign_to_agent: null });
    const TotalLead = lead.length;

    array.push({ ["name"]: "UnAssigned Lead", ["Value"]: TotalLead });
    res.status(201).json({
      success: true,
...
exports.AgentWishLeadCount1 = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch lead count" });
  }
});
...
exports.DashboardLeadCount = catchAsyncErrors(async (req, res, next) => {
  ...
  const meetinglead_name = await Status.findOne({
    _id: "65a904164473619190494480",
  ...
  const Re_Visit_name = await Status.findOne({
    _id: "65a903ca4473619190494478",
  });
  ///// for Call Back (Re-Visit)
  ...
  const Shedule_Visit_name = await Status.findOne({
    _id: "65a903e9447361919049447a",
  });
  array.push(
    { ["name"]: "Followup Lead", ["Value"]: followuplead.length },
    { ["name"]: "Total Agent", ["Value"]: TotalAgent },
    {
      ["name"]: meetinglead_name?.status_name1,
      ["Value"]: meetinglead.length,
      ["Value1"]: meetingleadNextDay.length,
      ["id"]: "65a904164473619190494480",
    },
    {
      ["name"]: Visit_name?.status_name1,
      ["Value"]: Visit.length,
      ["Value1"]: VisitleadNextDay.length,
      ["id"]: "65a903f8447361919049447c",
    },
    {
      ["name"]: Re_Visit_name?.status_name1,
      ["Value"]: Re_Visit.length,
      ["Value1"]: Re_VisitleadNextDay.length,
      ["id"]: "65a903ca4473619190494478",
    },
    {
      ["name"]: Shedule_Visit_name?.status_name1,
      ["Value"]: Shedule_Visit.length,
      ["Value1"]: Shedule_VisitleadNextDay.length,
...
exports.DashboardLeadCountOfUserByTeamLeader = catchAsyncErrors(
  async (req, res, next) => {
    const { user_id } = req.body;
    // const agents = await agent.find({ assigntl: user_id }).select('_id');
    // const agentIds = agents.map(agent => agent._id);
    ...
    const Visit_name = await Status.findOne({
      _id: "65a903f8447361919049447c",
    });
    ...
    array.push(
      ...
      {
        ["name"]: "Total Agent",
        ["Value"]: allAgents.map((agent) => new ObjectId(agent._id))?.length,
      },
      {
        ["name"]: meetinglead_name?.status_name1,
        ["Value"]: meetinglead.length,
