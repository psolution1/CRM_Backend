...
const ExcelUplode = async (req, res) => {
  try {
    ...
    const leadData = await csv().fromFile(req.file.path);
    const uniqueContactNos = [
      ...new Set(leadData.map((entry) => entry.contact_no)),
    ];
    const existingLeads = await Lead.find({
      contact_no: { $in: uniqueContactNos },
    });
    const uniqueNewContactNos = uniqueContactNos.filter(
      (contactNo) =>
        !existingLeads.some((lead) => lead.contact_no === contactNo)
    );
    const filteredLeadData = leadData.filter((entry) =>
      uniqueNewContactNos.includes(entry?.contact_no)
    );

    const insertedLeads = await Lead.insertMany(
      filteredLeadData.map((entry) => ({
        full_name: entry?.full_name,
        email_id: entry?.email_id,
        contact_no: entry?.contact_no,
        alternative_no: entry?.alternative_no,
    ...
    const insertedLeads = await Lead.insertMany(
      leadData.map((entry) => {
        ...
        if (entry?.interested_not_intrested) {
          description +=
            (description ? ", " : "") + entry.interested_not_intrested;
        }
...
const ExcelUplode111 = async (req, res) => {
  try {
    ...
    const newLeadsData = leadData
      .filter((entry) => uniqueNewContactNos.includes(entry.contact_no))
      .map((entry) => {
        let description = "";
